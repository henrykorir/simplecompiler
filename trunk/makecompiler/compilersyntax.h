...
struct prodinfo_t
{
	//tstring name;
	//tstring type;
	//int32 pid; // id of the production
	//production basicinfo; // production's basic information
	std::vector<tstring> content; // production's content
};
...
public:
	...
	typedef syntax::prodinfo_t prodinfo_t;
public:
	compiler_grammar();
	~compiler_grammar();
public:
	...
	int32 make_new_keywords(const tstring& keyword);
...
private:
	int32 make_terminate(const tstring& sname, const tstring& regexstring, const tstring& stype, bool is_direct);
